...
export default function Popup() {
  ...
  useEffect(() => {
    if (popupData?.artist !== undefined) {
      const normalizedArtist = {
        ...popupData.artist,
        image: normalizeImageUrl(popupData.artist.image),
      };
      setSelectedArtist(normalizedArtist);
      setFormData((prev) => ({ ...prev, artist: normalizedArtist }));
    }
  ...
  return (
    <div
      className={`fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 transition-opacity z-[99] ${
      ...
      {submissionStatus === "success" && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[1000]">
          <div className="bg-white p-6 rounded-lg text-center">
            <h2 className="text-xl font-bold text-black">Request Sent!</h2>
            ...
            <div className="popup flex flex-col md:flex-row h-full w-full max-h-[700px] md:items-stretch overflow-hidden rounded-[20px]">
              ...
              >
                {selectedArtist ? (
                  ...
                  <div className="w-full h-full flex flex-col justify-center items-center max-md:pt-10 max-md:pb-5">
